 ...
 *                                                                 
 * Template Thanks to  John Urbanic, PSC 2014
 *
 *******************************************************************/
...
MPI_Comm_rank(MPI_COMM_WORLD, &my_PE_num);
	...
	//Send bottom real row to top ghost row
	if(my_PE_num!=NPES-1){
		MPI_Recv(&Temperature[0][1], COLUMNS, MPI_DOUBLE, my_PE_num-1  DOWN, MPI_COMM_WORLD)
		MPI_Ssend(&Temperature[ROWS][1], COLUMNS, MPI_DOUBLE, my_PE_num+1, DOWN, MPI_COMM_WORLD)
	}
	//Send top real row to bottom ghost row 
